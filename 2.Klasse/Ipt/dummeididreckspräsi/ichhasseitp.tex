...
\begin{frame}{Windows und Closed Source}
	\only<0->{\begin{itemize}
			\only<0->{\item \say{Windows wird mit jedem Update schlechter} - jeder}
			      \only<1>{\item Werbungen im Startmenü\cite{Windoof1}}
			      \only<1>{\item Werbungen im Startmenü aber anders\cite{Windoof2}}
			      \only<1>{\item Werbungen im File Explorer\cite{Windoof3}}
			      \only<1>{\item Werbungen im File Explorer hätten wir anscheinend nicht sehen dürfen\cite{Windoof4}}
			      \only<1>{\item Mehr Werbungen\cite{Windoof5}\cite{Windoof6}}
			      \only<1>{\item TPM erforderlich\cite{Windoof7}}
			      \only<1>{\item Microsoft Account benötigt\cite{Windoof8}}
			      \only<1>{\item und noch VIELES mehr}
			      \only<2->{\item Nutzer muss mit Entscheidungen des Entwicklers leben}
			      \only<3->{\item Quellcode kann verloren werden}
			      \only<4->{\item Nutzer weiß nicht was die Software macht}
			      \only<5->{\item Nur die Firma kann die Sicherheit überprüfen}
			      \only<6->{\item Produkt kann nicht von der Community am Leben erhalten werden}
		\end{itemize}}
\end{frame}

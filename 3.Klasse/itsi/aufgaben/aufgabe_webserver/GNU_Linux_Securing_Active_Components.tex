...
\end{lstlisting}
\newpage
If we reload the Nginx configuration, our browser is going to give us a security warning since it recognizes that the certificate was not signed by a trusted organization but by ourselves. Also, this screenshot was taken after I set up the domain, but it doesn't matter if the IP or the domain is displayed here.
\begin{figure}[!htbp]
	\includegraphics[scale=0.25]{images/badgoober.png}
	\centering
	\caption{Browser warning for untrusted certificate}
\end{figure}
If we accept the risk and continue, we can inspect the certificate by clicking on the lock or settings symbol in the address bar of the browser, and then selecting "View Certificate." How you access the certificate may vary from browser to browser, though.
\begin{figure}[!htbp]
